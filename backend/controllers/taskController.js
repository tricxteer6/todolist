...
function validateTask(body) {
  const { date, task, priority, deadline, status } = body;
...
async function createTask(req, res, next) {
  ...
  try {
    const { date, task, priority, deadline, status } = req.body;
    const [result] = await pool.execute(
      'INSERT INTO tasks (user_id, date, task, priority, deadline, status) VALUES (?, ?, ?, ?, ?, ?)',
      [req.user.id, date, task.trim(), priority, normalizeDeadlineValue(deadline), status]
    );
...
async function updateTask(req, res, next) {
  ...
  try {
    const { date, task, priority, deadline, status } = req.body;
    const [result] = await pool.execute(
      'UPDATE tasks SET date = ?, task = ?, priority = ?, deadline = ?, status = ? WHERE id = ? AND user_id = ?',
      [date, task.trim(), priority, normalizeDeadlineValue(deadline), status, req.params.id, req.user.id]
    );
